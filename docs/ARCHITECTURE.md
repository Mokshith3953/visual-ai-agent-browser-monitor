...
a timer, ships each screenshot to a backend where **Claude vision** interprets what
...
│                              │ poll `pending`                                          │
│                              ▼                                                         │
│                  Vision worker ── analyzeScreenshot ──► Claude (server-side key)       │
│                    forced record_activity tool → {app, task, category, entities, …}   │
└──────────────────────────────┬────────────────────────────────────────────────────────┘
                              ▼
...
| Decision | Rationale |
| --- | --- |
| **API key server-side only** | The Anthropic key never ships in the extension. All vision runs on the backend. This is the single most important security choice. |
...
| **Forced tool-use for vision output** | Guarantees schema-valid JSON instead of parsing free-form text. |
...
7. The vision worker polls `pending`, downloads the image, calls Claude, writes `{app, task, category, entities, summary, containsSensitive}` and marks `processed`.
...
- Tune `VISION_MODEL` (`claude-opus-4-8` → `claude-sonnet-5` / `claude-haiku-4-5`) and the extension's `captureIntervalSec` to trade cost vs. resolution.
